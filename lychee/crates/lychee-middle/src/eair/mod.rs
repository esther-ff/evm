mod private {
    lychee_util::newtyped_index!(LocalId, LocalMap, LocalVec, LocalSlice);
    lychee_util::newtyped_index!(ParamId, ParamMap, ParamVec, ParamSlice);
...
use lychee_air::node::{self, StmtKind};
use lychee_util::def::{DefId, DefType, Resolved};

use crate::pill;
use crate::typing::fun_cx::FieldId;
use crate::typing::ty::{Instance, TyKind};
use lychee_air::lowering_ast::AirId;
use lychee_air::node::{AirLiteral, Constant, Expr as AirExpr, ExprKind::*, Local};
use lychee_ast::{self as ast, UnaryOp};
use lychee_util::arena::Arena;
use lychee_util::id::IdxVec;
use lychee_util::symbols::SymbolId;

use crate::session::Session;
use crate::typing::{fun_cx::TypeTable, ty::Ty};
...
impl<'ir> EairBuilder<'ir> {
    ...
    fn lower_expr(&mut self, expr: &AirExpr<'_>) -> Expr<'ir> {
        ...
        match expr.kind {
            ...
            lychee_air::node::ExprKind::Block { label, body } => Expr {
