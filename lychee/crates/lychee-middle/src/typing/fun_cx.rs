...
use crate::typing::ty::{InferKind, InferTy, LambdaEnv, ParamTy, Ty, TyKind, TypingError};
use lychee_air::node::{
    AirLiteral, BindItem, BindItemKind, Block, Expr, ExprKind, Lambda, Local, Path, Stmt, StmtKind,
    ThingKind, Universe,
};
use lychee_air::visitor::AirVisitor;
use lychee_air::{lowering_ast::AirId, node};
use lychee_ast::{BinOp, UnaryOp};
...
lychee_util::newtyped_index!(FieldId, FieldMap, FieldVec, FieldSlice);
lychee_util::newtyped_index!(InferId, InferMap, InferVec, InferSlice);
...
impl<'ty> TyCollector<'ty> {
    fn resolve_type_variable(&self, ty: Ty<'ty>) -> Ty<'ty> {
        match *ty {
            ...
            // get rid maybe?
            TyKind::Array(inner) if inner.is_infer() => self
                .sess
    ...
    fn visit_expr(&mut self, expr: &'vis Expr<'vis>) -> Self::Result {
        ...
        let ty = match *ty {
            ...
            TyKind::Lambda(lambda) => {
                let ty = if lambda.all_inputs.iter().any(|ty| ty.is_infer()) {
        ...
        lychee_air::visitor::walk_expr(self, expr);
    }
}
