...
use crate::typing::fun_cx::{FieldId, FieldSlice, InferId};
use core::panic;
use lychee_air::lowering_ast::AirId;
use lychee_air::node::{Constant, Node};
use lychee_util::def::{DefId, IntTy};
use lychee_util::errors::TheresError;
use lychee_util::pooled::Pooled;
...
pub fn instance_def<'cx>(cx: &'cx Session<'cx>, def_id: DefId) -> Instance<'cx> {
    use lychee_air::node::Field;
    use lychee_util::id::IdxSlice;
...
pub fn fn_sig_for<'cx>(cx: &'cx Session<'cx>, def_id: DefId) -> FnSig<'cx> {
    ...
    match cx.def_type(def_id) {
        ...
        DefType::NativeFn => {
            ...
            match item.kind {
                lychee_air::node::NativeItemKind::Fun { args, ret } => FnSig {
                    inputs: cx
                        .arena()
