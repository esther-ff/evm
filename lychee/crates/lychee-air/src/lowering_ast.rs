...
use crate::node::{self, Constant, Lambda, Node, Param};
use lychee_util::def::{BodyId, BodyVec, DefId, DefMap, DefPath, DefType, DefVec, Resolved};
use lychee_util::id::IdxVec;
use lychee_util::span::Span;
use lychee_util::symbols::SymbolId;

lychee_util::newtyped_index!(AirId, AirIdMap, AirVec);
lychee_util::newtyped_index!(LocalId, LocalMap, LocalVec);
...
impl<'air> AirBuilder<'air> {
    ...
    pub fn lower_universe(
        ...
        let node::ThingKind::Realm { name: _, things } = self.lower_realm(&universe.realm) else {
            unreachable!()
        };

        let universe = node::Universe::new(things);
