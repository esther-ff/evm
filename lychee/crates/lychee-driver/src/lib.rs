...
use lychee_ast::pretty::PrettyPrinter;
use lychee_lexer::{Lexemes, Lexer};
use lychee_middle::session::Session;
use lychee_middle::typing::fun_cx::typeck_universe;
use lychee_util::arena::Arena;
use lychee_util::errors::DiagEmitter;
use lychee_util::flags::Flags;
use lychee_util::sources::{FileManager, SourceId, Sources};
...
impl Compiler {
        ...
        A: AsRef<Path>,
    {
        ...
        let arena = Arena::new();
        let (uni, map) = lychee_air::lower_universe(&arena, &diags, &ast);

        Session::new(&diags, self.flags, &self.sources, &arena, map).enter(|session| {
            lychee_air::dump_air(&mut stdout(), self.flags.dump_hir, uni, session.air_map())
                .unwrap();
            typeck_universe(session, uni);
            let main_did = lychee_middle::passes::check::check_for_main(session, uni)
                .expect("todo: no main lmao!");
            ...
            lychee_middle::dataflow::analyze_maybe_init_variables(pill.cfg());
    ...
    fn parse_to_ast<'a>(&self, lexemes: Lexemes, diag: &'a DiagEmitter<'a>) -> Universe {
        let decls = lychee_parser::parse(lexemes, diag);
