...
use lychee_ast::*;
use lychee_lexer::{Lexemes, Token, TokenKind};
use lychee_util::errors::{DiagEmitter, TheresError};
use lychee_util::span::Span;
...
impl TheresError for ParseError {
    fn message(&self) -> Cow<'static, str> {
        match self {
            ParseError::UselessVisibility => "this visibility declaration is useless".into(),
...
pub fn parse<'a>(lexemes: Lexemes, diag: &'a DiagEmitter<'a>) -> Universe {
    ...
    while !parser.lexemes.is_empty() {
        let vis = parser.visibility();
        let id = parser.new_id();
        match parser.declaration() {
            Err(..) => {}
            Ok(kind) => {
                if matches!(kind, ThingKind::Bind(..))
                    && let Visibility::Public(sp) = vis
                {
                    diag.emit_err(ParseError::UselessVisibility, sp);
                }

                parser.decls.push(Thing { kind, id, vis })
            }
        }
    }
...
impl Parser<'_> {
    ...
    fn declaration(&mut self) -> Result<ThingKind> {
        ...
        let decl = match tok.kind {
            ...
            TokenKind::Realm => self.realm_decl().map(ThingKind::Realm)?,
            TokenKind::Eof => {
    ...
    fn realm_decl(&mut self) -> Result<Realm> {
    ...
    fn visibility(&mut self) -> Visibility {
        ...
        if let TokenKind::Pub = peeked.kind {
            self.lexemes.advance();
            return Visibility::Public(peeked.span);
        }
    ...
    fn statement(&mut self) -> Result<ExprOrStmt> {
        ...
        let stmt = match tok.kind {
            ...
            TokenKind::Function => StmtKind::Thing(Thing {
                kind: self.declaration()?,
                id,
                vis: Visibility::None,
            }),

            TokenKind::Pub if self.lexemes.peek2().kind == t!(Function) => {
                let vis = self.lexemes.next_token();
                assert_eq!(vis.kind, TokenKind::Pub);

                StmtKind::Thing(Thing {
                    kind: self.declaration()?,
                    id,
                    vis: Visibility::Public(vis.span),
                })
            }

            TokenKind::Pub => {
                self.lexemes.advance();
                self.diag.emit_err(ParseError::UselessVisibility, tok.span);
                return Ok(ExprOrStmt::Expr(Expr::new(
                    ExprType::Err,
                    tok.span,
                    self.new_id(),
                )));
